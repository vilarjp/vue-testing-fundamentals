...
describe('ProductCard - Unit', () => {
  ...
  it('should add item to CartManager on button click', async () => {
    ...
    const cartManagerAddProductSpy = jest.spyOn(cartManager, 'addProduct');

    await wrapper.find('button').trigger('click');

    expect(cartManager.getState().items).toHaveLength(1);
    expect(cartManager.getState().open).toBe(true);
    expect(cartManagerAddProductSpy).toHaveBeenCalledWith(product);
  });
});
